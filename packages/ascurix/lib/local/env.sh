...
write_backend_env() {
  ...
  # on a non-default worktree host/port silently resolves against the wrong
  # origin.
  set_env_var "$file" "AUTH_URL" "http://${ip}:3000"
  set_env_var "$file" "NEXTAUTH_URL" "http://${ip}:3000"
