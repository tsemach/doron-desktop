...
export default function EmailsArrivedCard({ emails }: EmailsArrivedCardProps) {
  ...
  return (
    <DashboardCard
      ...
      title={t("dashboard_emails_arrived_title")}
      count={emails.length}
      column={2}
    >
      {spotlight && (
        <div className="rounded-xl border border-indigo-200 bg-indigo-50 p-3">
          ...
          <p className="mt-1 text-xs text-indigo-700">
            {spotlight.matchStatus === "matched" ? spotlight.matchedCaseSubject : t("dashboard_needs_review")}
          </p>
        </div>
      )}
      {rest.length > 0 && (
            ...
            <li key={email.id}>
              <StatusRail color={email.matchStatus === "matched" ? "blue" : "amber"}>
                ...
                <p className="text-xs text-muted-foreground">
                  {email.matchStatus === "matched" ? email.matchedCaseSubject : t("dashboard_needs_review")}
                </p>
              </StatusRail>
            </li>
          ))}
