...
graph TD
    ...
    %% Elements
    A[User Input: Raw Search Query<br><i>indexStore.ts</i>]:::grayBox --> B[Query Analysis Phase<br><i>mod.rs: query_search_documents_core</i>]:::grayBox
    ...
    subgraph Query Analysis [1. Query Analysis - src/query/llm.rs]
        ...
        C -->|No| E[Cloud LLM Call<br><i>llm.rs: query_llm_analyze_query</i>]:::grayBox
        D --> F[Structured Query Parameters<br><i>types.rs: QueryAnalysis</i>]:::grayBox
        E --> F
    end

    F --> G[Dual-Track Retrieval Phase<br><i>queries.rs: query_smart_execute</i>]:::grayBox
    ...
    subgraph Dual Retrieval [2. Dual-Track Retrieval - src/query/queries.rs]
        ...
        I --> K[Generate Query Embeddings<br><i>embeddings.rs: embedding_by_query</i>]:::grayBox
    ...
    P --> Q[Candidate Reranking Phase<br><i>mod.rs: query_search_documents_core</i>]:::grayBox

    subgraph Reranking [4. Candidate Reranking - src/query/llm.rs]
        Q --> R[Format RERANK_PROMPT with candidates metadata]:::grayBox
        R --> S[Query LLM for Reranking<br><i>llm.rs: query_llm_rerank_candidates</i>]:::grayBox
...
*   **Core Functions**: 
    *   `query_search_documents_core` parses options to choose the analysis path.
    *   `llm::query_llm_analyze_query` invokes the configured AI model with `QUERY_ANALYSIS_PROMPT` to request structured extraction.
...
*   **Core Functions**:
    *   `queries::query_smart_execute` dispatches the database requests.
    *   **Track 1 (Text Match)**: SQLite queries run FTS5 `MATCH` queries over extracted keywords, scoring matches with `bm25()`.
    *   **Track 2 (Semantic Vector)**: `embeddings::embedding_by_query` computes a 384-dimensional query vector using the native `fastembed` model, then executes an SQLite math query evaluating cosine similarity between the query vector and indexed document chunk vectors.
...
*   **Core Functions**:
    *   `queries::query_smart_execute` applies the SQLite `WHERE` constraints for metadata filtering (using `doc_type` and `doc_date` from query analysis).
...
*   **Core Functions**:
    *   `llm::query_llm_rerank_candidates` formats the candidate rows (ID, title, summary) into `RERANK_PROMPT` as a JSON string.
    ...
    *   Returns the reordered `Vec<DocumentRow>` results to `query_search_documents_core` which returns them to the UI frontend.
