...
New module `apps/desktop/src-tauri/src/calendar/oauth.rs`, reusing the **exact** redirect mechanism already built for backend login (`auth/mod.rs`):

1. User clicks "Connect Google Calendar" in the Calendar tab (shown whenever `google_calendar_accounts` is empty).
2. Rust opens the system browser to Google's OAuth consent URL (`scope=https://www.googleapis.com/auth/calendar`, `redirect_uri=doron-desktop://oauth/google-calendar`), using the OAuth client already provisioned (see the credential files flagged in `why-and-requirements.md`; the client ID/secret move into the same config surface `llm_settings.rs` uses for provider keys, not hardcoded in source).
3. Google redirects back to `doron-desktop://oauth/google-calendar?code=...`, caught by the same `app.deep_link().on_open_url(...)` handler in `lib.rs::setup()` that already exists for backend auth (a second `match` arm on the URL path, not a new listener).
4. Rust exchanges `code` for `access_token`/`refresh_token` via `reqwest::Client::post("https://oauth2.googleapis.com/token", ...)` — same call shape as `auth::login_with_credentials`.
...
- **OAuth client credential storage**: §4 assumes the Google OAuth client ID/secret move into the same settings surface as LLM provider keys; the exact mechanism (bundled with the app vs. per-install config) is a decision for the PR that implements §4, informed by how the existing loose credential files (flagged in `why-and-requirements.md`) were meant to be used.
